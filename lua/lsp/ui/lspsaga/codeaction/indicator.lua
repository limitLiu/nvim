...
if vim.tbl_isempty(vim.fn.sign_getdefined(M.sign_name)) then
  vim.fn.sign_define(
    M.sign_name,
    { text = icon, texthl = "LspSagaLightBulbSign" }
  )
end
...
M.update = function(winid, line)
  ...
  if config.sign then
    ...
    if window[winid].lightbulb_line ~= 0 then
      vim.fn.sign_unplace(
        M.group,
        { id = window[winid].lightbulb_line, buffer = "%" }
      )
    end
...
M.check = function()
  ...
  api.code_action_request {
    ...
    callback = function(ctx)
      ...
      local fail_to_have_diagnostics = M.require_diagnostics[vim.bo.filetype]
        and ctx.params.diagnostics
        and next(ctx.params.diagnostics) == nil
...
M.attach = function()
  vim.cmd [[autocmd CursorHold,CursorHoldI * lua require'lsp.ui.lspsaga.codeaction.indicator'.check()]]
end
