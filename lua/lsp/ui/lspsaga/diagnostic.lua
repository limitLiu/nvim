...
local wrap = require "lsp.ui.lspsaga.wrap"
local config = require("lsp.ui.lspsaga").config_values
local if_nil = vim.F.if_nil
local hover = require "lsp.ui.lspsaga.hover"
...
local show_diagnostics = function(opts, get_diagnostics)
  ...
  if has_var and diag_float_winid ~= nil then
    if
      ...
      and vim.api.nvim_win_is_valid(diag_float_winid[2])
    then
      return
    end
  end
  ...
  for i, diagnostic in ipairs(diagnostics) do
    ...
    local prefix = string.gsub(
      config.diagnostic_prefix_format,
      "%%s",
    ...
    local wrap_message = wrap.wrap_contents(
      message_lines,
      max_width,
      { fill = true, pad_left = #prefix }
    )
...
M.navigate = function(direction)
  return function(opts)
    ...
    if not pos then
      ...
      return print(
        fmt(
          "Diagnostic%s: No more valid diagnostics to move to.",
