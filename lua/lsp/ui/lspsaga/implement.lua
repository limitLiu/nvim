...
local action = require "lsp.ui.lspsaga.action"
local implement = {}
...
function implement.lspsaga_implementation(timeout_ms)
  ...
  if vim.tbl_islist(result) and not vim.tbl_isempty(result[1]) then
    ...
    content = vim.list_extend(
      { config.definition_preview_icon .. "Lsp Implements", "" },
      content,
      1,
      #content
    )
...
function implement.scroll_in_implement(direction)
  local has_hover_win, data = pcall(
  ...
  current_win_lnum = action.scroll_in_win(
    hover_win,
    direction,
    current_win_lnum,
    last_lnum,
