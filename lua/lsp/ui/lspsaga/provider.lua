local window = require "lsp.ui.lspsaga.window"
local vim, api, lsp, vfn = vim, vim.api, vim.lsp, vim.fn
local config = require("lsp.ui.lspsaga").config_values
local libs = require "lsp.ui.lspsaga.libs"
local home_dir = libs.get_home_dir()
local scroll_in_win = require("lsp.ui.lspsaga.action").scroll_in_win
...
function Finder:lsp_finder_request()
  return uv.new_async(vim.schedule_wrap(function()
    ...
    if string.len(root_dir or "") == 0 then
...
function Finder:create_finder_contents(result, method_type, root_dir)
  ...
  if type(result) == "table" then
    ...
    local title = method_option[method_type].icon
      .. params
      .. method_option[method_type].title
...
function Finder:render_finder_result()
  ...
  api.nvim_command 'autocmd CursorMoved <buffer> lua require("lsp.ui.lspsaga.provider").set_cursor()'
  api.nvim_command 'autocmd CursorMoved <buffer> lua require("lsp.ui.lspsaga.provider").auto_open_preview()'
  api.nvim_command "autocmd QuitPre <buffer> lua require('lsp.ui.lspsaga.provider').close_lsp_finder_window()"
...
function Finder:apply_float_map()
  ...
  local nvim_create_keymap = require("lsp.ui.lspsaga.libs").nvim_create_keymap
  local lhs = {
    noremap = true,
    silent = true,
  }
  local keymaps = {
    {
      ...
      "n",
      action.vsplit,
      ":lua require'lsp.ui.lspsaga.provider'.open_link(2)<CR>",
    },
    {
      self.bufnr,
      "n",
      action.split,
      ":lua require'lsp.ui.lspsaga.provider'.open_link(3)<CR>",
    },
    {
      self.bufnr,
      "n",
      action.scroll_down,
      ":lua require'lsp.ui.lspsaga.provider'.scroll_in_preview(1)<CR>",
    },
    {
      self.bufnr,
      "n",
  ...
  if type(action.open) == "table" then
    for _, key in ipairs(action.open) do
      table.insert(keymaps, {
        ...
        "n",
        key,
        ":lua require'lsp.ui.lspsaga.provider'.open_link(1)<CR>",
      })
    end
  ...
  if type(action.quit) == "table" then
    for _, key in ipairs(action.quit) do
      table.insert(keymaps, {
        ...
        "n",
        key,
        ":lua require'lsp.ui.lspsaga.provider'.close_lsp_finder_window()<CR>",
      })
    end
...
function Finder:auto_open_preview()
  ...
  if next(content) ~= nil then
    ...
    if max_width > 42 then
      ...
      local max_height = self.WIN_HEIGHT
        - finder_win_opts.row
        - finder_height
        - border_width
        - 2
    ...
    vim.defer_fn(function()
      ...
      api.nvim_win_set_var(
        0,
        "saga_finder_preview",
        { winid, 1, config.max_preview_lines + 1 }
      )
    end, 10)
  end
end
...
function Finder:open_link(action_type)
  ...
  vim.fn.cursor {
    self.short_link[current_line].row,
    self.short_link[current_line].col,
  }
  self:clear_tmp_data()
end
...
function Finder:scroll_in_preview(direction)
  ...
  current_win_lnum = scroll_in_win(
    pdata[1],
    direction,
    current_win_lnum,
    last_lnum,
  ...
  api.nvim_win_set_var(
    0,
    "saga_finder_preview",
    { pdata[1], current_win_lnum, last_lnum }
  )
end
  ...
  if vim.tbl_islist(result) then
    for _, definition in ipairs(result) do
      ...
      then
        ...
        -- reduce filename length by root_dir or home dir
        if link:find(root_dir or "", 1, true) then
          short_name = link:sub((root_dir or ""):len() + 2)
        elseif link:find(home_dir, 1, true) then
          short_name = link:sub(home_dir:len() + 2)
          -- some definition still has a too long path prefix
          if #short_name > 40 then
        ...
        local range = definition.result[1].targetRange
          or definition.result[1].range
        local start_line = 0
        ...
        content = vim.list_extend({
          config.definition_preview_icon
            .. "Definition Preview: "
            .. short_name,
          "",
        }, content, 0, #content)
        ...
        libs.close_preview_autocmd(
          { "CursorMoved", "CursorMovedI", "BufHidden", "BufLeave" },
          wi
        )
        ...
        api.nvim_buf_set_var(
          0,
          "lspsaga_def_preview",
          { wi, 1, config.max_preview_lines, 10 }
        )
      end
    end
  end
...
function lspfinder.scroll_in_def_preview(direction)
  ...
  local current_win_lnum = scroll_in_win(
    pdata[1],
    direction,
    pdata[2],
    config.max_preview_lines,
