...
local apply_action_keys = function()
  ...
  api.nvim_command(
    "inoremap <buffer><nowait><silent>"
      .. exec_key
      .. ' <cmd>lua require("lsp.ui.lspsaga.rename").do_rename()<CR>'
  )
  if type(quit_key) == "table" then
    for _, k in ipairs(quit_key) do
      api.nvim_command(
        "inoremap <buffer><nowait><silent>"
          .. k
          .. ' <cmd>lua require("lsp.ui.lspsaga.rename").close_rename_win()<CR>'
      )
    end
  else
    api.nvim_command(
  ...
  api.nvim_command 'nnoremap <buffer><silent>q <cmd>lua require("lsp.ui.lspsaga.rename").close_rename_win()<CR>'
end
...
local rename = function()
  ...
  local saga_rename_prompt_prefix =
    api.nvim_create_namespace "lspsaga_rename_prompt_prefix"
  ...
  api.nvim_command "autocmd QuitPre <buffer> ++nested ++once :silent lua require('lsp.ui.lspsaga.rename').close_rename_win()"
  apply_action_keys()
end
