...
local function open_shadow_win()
  ...
  local shadow_winhl =
    "Normal:SagaShadow,NormalNC:SagaShadow,EndOfBuffer:SagaShadow"
...
function M.create_win_with_border(content_opts, opts)
  ...
  api.nvim_win_set_option(
    winid,
    "winhl",
    "Normal:LspFloatWinNormal,FloatBorder:" .. highlight
  )
...
function M.fancy_floating_markdown(contents, opts)
  ...
  local function apply_syntax_to_region(ft, start, finish)
    ...
    if
      not pcall(
        vim.cmd,
        string.format("syntax include %s syntax/%s.vim", lang, ft)
      )
    then
      return
    end
