...
LspProgress.init = function(self, options)
  ...
  self.options.colors = vim.tbl_extend(
    "force",
    LspProgress.default.colors,
    self.options.colors or {}
  )
  self.options.separators = vim.tbl_deep_extend(
    "force",
    LspProgress.default.separators,
    self.options.separators or {}
  )
  self.options.display_components = self.options.display_components
    or LspProgress.default.display_components
  self.options.timer =
    vim.tbl_extend("force", LspProgress.default.timer, self.options.timer or {})
  self.options.spinner_symbols = vim.tbl_extend(
    "force",
    LspProgress.default.spinner_symbols,
    self.options.spinner_symbols or {}
  )
  self.options.message = vim.tbl_extend(
    "force",
    LspProgress.default.message,
    self.options.message or {}
  )
  ...
  if self.options.colors.use then
    ...
    self.highlights.lsp_client_name =
  ...
  self.progress_callback = function(_, msg, info)
    ...
    if key then
      ...
      if progress_collection[key] == nil then
        progress_collection[key] =
          { title = nil, message = nil, percentage = nil }
      end
  ...
  for _, client in pairs(self.clients) do
    for _, display_component in pairs(self.options.display_components) do
      ...
      if type(display_component) == "table" then
        self:update_progress_components(
          result,
          display_component,
          client.progress
        )
...
LspProgress.update_progress_components = function(
  self,
  result,
  display_components,
  client_progress
